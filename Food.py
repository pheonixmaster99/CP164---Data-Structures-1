...
class Food:
    ...
    def origins():
        ...
        # your code here
        origins_list = [f"{index} {origin}" for index,
                        origin in enumerate(Food.ORIGIN)]
        string = '\n'.join(origins_list)
    ...
    def __str__(self):
        ...
        string = (f"Name: {self.name: <20}\n"
                  f"Origin: {countries_dict.get(int(self.origin)): <20}\n"
                  f"Vegetarian: {str(self.is_vegetarian): <10}\n"
                  f"Calories: {str(self.calories): <10}")
