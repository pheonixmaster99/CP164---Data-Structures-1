...
class List:
    ...
    def _is_identical_r_aux(self, source_node, target_node):
        ...
        if source_node is None and target_node is None:
            identical = True
        elif source_node is None or target_node is None or source_node._value != target_node._value:
            identical = False
        else:
            # General Case: the values are matched between two List objects, and going towards base case with both the
            # ends of the List objects being none
    ...
    def _linear_search_r_aux(self, previous, current, index, key):
        ...
        # Base Case: The value of the List toward the end is None
        if current is None:
            result = (None, None, -1)
        elif current._value == key:
            result = (previous, current, index)
        else:
            result = self._linear_search_r_aux(
    ...
    def _intersection_r_aux(self, source1_node, source2):
        if source1_node is not None:
            # Check if the value is in source2 and not already in self
            _, found_node, _ = source2._linear_search_r(source1_node._value)
            if found_node is not None:
                # Check if the value is not already in the target list
                _, in_self, _ = self._linear_search_r(source1_node._value)

                if in_self is None:
                    # Append to self if it's not already there
                    self.append(source1_node._value)

            # Move to the next node in source1
            self._intersection_r_aux(source1_node._next, source2)
        # val = source1_node._value
        # # Base Case: The value is None at the end of the linked_list
        # if source1_node is None:
        #     self.append(val)
        # else:
        #     # Check if the value in source1 is also in source2
        #     if source1_node is not None:
        #         _, current, _ = source2._linear_search_r(val)
        #     else:
        #         # Append the current list if val in source 1 and source2
        #         self.append(val)
        #
        #     # Recursive Logic: Keep going through the linked list and checking for values in source 1 and source2
        #     self._intersection_r_aux(current._next, source1_node.next, source2)
